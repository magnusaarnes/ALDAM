...
def detect_color_deviancies(frame, blur_strengths=(0.1,0.04), color_object_lower_thresh=0.3, booster_threshold=0.01,bilateral_filter_gains=(0.027,0.14375,0.14375), threshold_preblur_rad=(5,5)):
    ...
    stage[2] = (col_normd.copy()*255).astype(np.ubyte)

    # For each channel, compute pixel-wise distance from blurred normals
    color_deviancy = spectral_autodifference(col_normd, blur_strengths[0])
    stage[3] = (color_deviancy.copy()*255).astype(np.ubyte)
    ...
    stage[4] = processed_deviancy.copy()
    ...
    stage[5] = threshed_detections.copy()
    ...
    stage[6] = threshed_detections.copy()
    ...
    for ni in range(3):
        stage[4+ni] = (stage[4+ni]*255).astype(np.ubyte)
        stage[4+ni] = cv2.cvtColor(stage[4+ni], cv2.COLOR_GRAY2BGR)
    stage[4] = cv2.applyColorMap(stage[4], cv2.COLORMAP_INFERNO)

    hcon1 = cv2.hconcat([stage[0], stage[1], stage[2], stage[3]])
    hcon2 = cv2.hconcat([stage[4], stage[5], stage[6], stage[7]])
    displ = cv2.vconcat([hcon1, hcon2])

    # Downsize concat image
    displ = cv2.resize(displ, (int(480*4), int(270*2)))
...
for i in range(len(images)):
    ...
    detections, centroids, concats, threshed_detects = detect_color_deviancies(image, blur_strs, color_object_lower_thresh, booster_threshold,bilateral_filter_gains=(neighborhood_gain,sigmaColor_gain,sigmaSpace_gain), threshold_preblur_rad=threshold_preblur_rad)
    print("Detected objects: ", np.shape(centroids)[1], "\n")

    # Save outputs
    im_name_conc = str ('debug_' + onlyfiles[i])
